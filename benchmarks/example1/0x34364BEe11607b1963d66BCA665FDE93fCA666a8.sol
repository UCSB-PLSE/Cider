...
contract YOUToken is StandardToken, Ownable {
  ...
  uint256 private constant INITIAL_SUPPLY = 32 * (10 ** 26);

  uint256 private constant ANGEL_SUPPLY                 = 32 * (10 ** 25);  // 10% for angel sale
  uint256 private constant PRIVATE_SUPPLY               = 64 * (10 ** 25);  // 20% for private sale
  uint256 private constant TEAM_SUPPLY                  = 48 * (10 ** 25);  // 15% for team
  uint256 private constant FOUNDATION_SUPPLY            = 80 * (10 ** 25);  // 25% for foundation
  uint256 private constant COMMUNITY_SUPPLY	            = 96 * (10 ** 25);  // 30% for community rewards
  
  uint256 private constant ANGEL_SUPPLY_VESTING         = 16 * (10 ** 25);    // 50% of angel sale
  struct VestingGrant {
        address beneficiary;
        uint256 start;
  ...
  function YOUToken() public {
    ...
    totalSupply = (((PRIVATE_SUPPLY + FOUNDATION_SUPPLY)) + COMMUNITY_SUPPLY);
    ...
    angelVestingGrant = VestingGrant(ANGEL_ADDRESS, now + 1 days, (30 days), ANGEL_SUPPLY_VESTING, 0, 4);
    teamVestingGrant = VestingGrant(TEAM_ADDRESS, now + 1 days, (30 days), TEAM_SUPPLY, 0, 60);
  }

  function releaseAngelFirstVested() public onlyOwner {
    require(!angelFirstVested && now >= angelVestingGrant.start);
    uint256 angelFirstSupplyBalance = (ANGEL_SUPPLY - ANGEL_SUPPLY_VESTING);
    totalSupply = (totalSupply + angelFirstSupplyBalance);
  ...
  function releaseAngelVested() public onlyOwner {
      uint256 amount = releasableAmount(now, angelVestingGrant);
      require(amount > 0);

      angelVestingGrant.transfered = ((angelVestingGrant.transfered) + amount);
      totalSupply = (totalSupply + amount);
      balances[angelVestingGrant.beneficiary] = ((balances[angelVestingGrant.beneficiary]) + amount);
      emit Transfer(address(0), angelVestingGrant.beneficiary, amount);
      // releaseVestingGrant(angelVestingGrant);
  }

  function releaseTeamVested() public onlyOwner {
      uint256 amount = releasableAmount(now, teamVestingGrant);
      require(amount > 0);

      teamVestingGrant.transfered = ((teamVestingGrant.transfered) + amount);
      totalSupply = (totalSupply + amount);
      balances[teamVestingGrant.beneficiary] = ((balances[teamVestingGrant.beneficiary]) + amount);
      emit Transfer(address(0), teamVestingGrant.beneficiary, amount);
  ...
  function releasableAmount(uint256 time, VestingGrant grant) internal pure returns (uint256) {
    ...
    uint256 amountPerRelease = ((grant.amount) / (grant.releaseCount));
    uint256 amount = (amountPerRelease * (((((time - (grant.start)))) / (grant.duration))));
    if (amount > grant.amount) {
    amount = grant.amount;
    }
    amount = (amount - (grant.transfered));
    return amount;
  }
  ...
  function releaseVestingGrant(VestingGrant storage grant) internal {
    ...
    grant.transfered = ((grant.transfered) + amount);
    totalSupply = (totalSupply + amount);
    balances[grant.beneficiary] = ((balances[grant.beneficiary]) + amount);
