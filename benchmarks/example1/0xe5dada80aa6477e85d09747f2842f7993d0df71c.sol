...
contract DockToken is StandardToken, Ownable {
    ...
    uint256 public constant TOKEN_OFFERING_ALLOWANCE = 1000000000 * (10 ** 18);
...
contract DockCrowdsale is Pausable {
    ...
    function startOffering(uint256 durationInSeconds) public onlyOwner atStage(Stages.Setup) {
        ...
        endTime = (extraTime + durationInSeconds);
    ...
    modifier validPurchase() {
        ...
        uint256 contributionInWei_ = msg.value;
        address participant_ = msg.sender; 


        require(contributionInWei_ <= (maxContribution - (contributions[participant_])));
        require(participant_ != address(0) && contributionInWei_ >= minContribution && contributionInWei_ <= maxContribution);
        require((weiRaised + contributionInWei_) <= FUNDING_ETH_HARD_CAP);
    ...
    */
    ...
    function buyTokens() internal /*validPurchase */{
        ...
        uint256 tokens = (contributionInWei * rate);
        
        if (!token.transferFrom(token.owner(), participant, tokens)) {
            revert();
        }

        weiRaised = (weiRaised + contributionInWei);
        contributions[participant] = ((contributions[participant]) + contributionInWei);

        remainCap = (FUNDING_ETH_HARD_CAP - weiRaised);
