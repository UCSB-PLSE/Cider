...
contract StarbaseToken is StandardToken {
    ...
    function StarbaseToken(
        ...
        balances[address(0)] = 62500000e18; // 62.5M
        ...
        returns (bool)
    {
        ...
        assert((now - (starbaseCrowdsale.endedAt())) >= 180 days);   // shall not be declared for 6 months after the crowdsale ended
        assert((unlockCompanysTokensAt - now) >= 60 days);   // tokens must be untransferable at least for 2 months
        ...
        if (publicOfferingPlans.length > 0) {
            ...
            assert((now - lastDeclaredAt) >= 180 days);
        }

        uint256 totalDeclaredTokenCount = tokenCount;
        for (uint8 i; i < publicOfferingPlans.length; i++) {
            totalDeclaredTokenCount = (totalDeclaredTokenCount + (publicOfferingPlans[i].tokenCount));
        }
        ...
        returns (bool)
    {
        ...
        initialEcTokenAllocation[to] =
            ((initialEcTokenAllocation[to]) + value);
        ...
        returns (bool)
    {
        ...
        balances[_for] = ((balances[_for]) + value);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        if (initialEcTokenAllocation[from] > 0) {
            untransferableTokenCount = (untransferableTokenCount + (numOfUntransferableEcTokens(from)));
        }

        // EP and CS purchasers' tokens should be untransferable initially
        if (starbaseCrowdsale.isEnded()) {
            uint256 passedDays =
                (now - (starbaseCrowdsale.endedAt())) / 86400; // 1d = 86400s
            if (passedDays < 7) {  // within a week
                // crowdsale purchasers cannot transfer their tokens for a week
                untransferableTokenCount = (untransferableTokenCount + (starbaseCrowdsale.numOfPurchasedTokensOnCsBy(from)));
            }
            if (passedDays < 14) {  // within two weeks
                // early purchasers cannot transfer their tokens for two weeks
                untransferableTokenCount = (untransferableTokenCount + (starbaseCrowdsale.numOfPurchasedTokensOnEpBy(from)));
            }
        }

        uint256 transferableTokenCount =
            ((balances[from]) - untransferableTokenCount);
    ...
    function numOfTransferableCompanysTokens() constant public returns (uint256) {
        ...
        for (uint8 i; i < publicOfferingPlans.length; i++) {
            ...
            if (plan.unlockCompanysTokensAt <= now) {
                unlockedTokens = (unlockedTokens + (plan.tokenCount));
            }
        }
        return ((balances[company]) - (initialCompanysTokenAllocation - unlockedTokens));
    }
    ...
    function numOfUntransferableEcTokens(address _for) constant public returns (uint256) {
        ...
        uint256 passedWeeks = (now - mvpLaunchedAt) / 7 days;
        ...
        if (transferableTokenCount >= initialCount) {
            ...
            return (initialCount - transferableTokenCount);
        }
    }
    ...
    function numOfInflatableTokens() constant public returns (uint256) {
        uint256 _endedAt = starbaseCrowdsale.endedAt();
        if (_endedAt == 0) {
            return 0;
        }
        uint256 passedDays = (now - _endedAt) / 86400;  // 1d = 60s * 60m * 24h = 86400s
        ...
        for (uint256 i; i < passedYears; i++) {
            inflatedSupply = (inflatedSupply + ((inflatedSupply * 25) / 1000)); // 2.5%/y = 0.025/y
        }

        uint256 remainderedDays = passedDays * 100 % 36525 / 100;
        if (remainderedDays > 0) {
            uint256 inflatableTokensOfNextYear =
                (inflatedSupply * 25) / 1000;
            inflatedSupply = (inflatedSupply + ((inflatableTokensOfNextYear * (remainderedDays * 100)) / 36525));
        }

        return (inflatedSupply - totalSupply);
    }
