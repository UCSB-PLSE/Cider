...
contract IERC721 is IERC165 {
    ...
    function safeTransferFrom4(address from, address to, uint256 tokenId, bytes memory data) public;
}
...
contract ERC165 is IERC165 {
     ...
     */
    mapping(uint32 => bool) private _supportedInterfaces;

    constructor () internal {
        // Derived contracts need only register support for their own interfaces,
        // we register support for ERC165 itself here
        // _registerInterface(_INTERFACE_ID_ERC165);
        _supportedInterfaces[uint32(_INTERFACE_ID_ERC165)] = true;
    }
     ...
     */
    function supportsInterface(bytes4 interfaceId) external view returns (bool) {
        return _supportedInterfaces[uint32(interfaceId)];
    }
    ...
    function _registerInterface(bytes4 interfaceId) internal {
        ...
        _supportedInterfaces[uint32(interfaceId)] = true;
    }
}
...
contract ERC721 is Context, ERC165, IERC721 {
     ...
     */
    function safeTransferFrom(address from, address to, uint256 tokenId) public {
        safeTransferFrom4(from, to, tokenId, "");
    }
     ...
     */
    function safeTransferFrom4(address from, address to, uint256 tokenId, bytes memory _data) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        _safeTransferFrom4(from, to, tokenId, _data);
    }
     ...
     * @param _data bytes data to send along with a safe transfer check
     */
    function _safeTransferFrom4(address from, address to, uint256 tokenId, bytes memory _data) internal {
        _transferFrom(from, to, tokenId);
     ...
     * @param _data bytes data to send along with a safe transfer check
     */
    function _safeMint3(address to, uint256 tokenId, bytes memory _data) internal {
        _mint(to, tokenId);
     ...
     */
    function _burn2(address owner, uint256 tokenId) internal {
    ...
    function _burn(uint256 tokenId) internal {
        _burn2(ownerOf(tokenId), tokenId);
    }
