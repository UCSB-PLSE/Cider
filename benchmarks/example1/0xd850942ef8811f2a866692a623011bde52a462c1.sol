...
contract VEN is Token, Owned {
    ...
    function claimBonus(address _owner) internal{      
        ...
        if (accounts[_owner].rawTokens != 0) {
            ...
            uint256 bonus = (((realBalance - (accounts[_owner].balance))) - (accounts[_owner].rawTokens));
    ...
    function balanceOf(address _owner) constant returns (uint256 balance) {
        ...
        if (bonusOffered > 0) {
            uint256 bonus = (((bonusOffered * (accounts[_owner].rawTokens))) / (supplies.rawTokens));

            return (((bonus + (accounts[_owner].balance))) + (accounts[_owner].rawTokens));
        }
        
        return ((uint256(accounts[_owner].balance)) + (accounts[_owner].rawTokens));
    }
    ...
    function transfer(address _to, uint256 _amount) returns (bool success) {
        ...
        if (accounts[msg.sender].balance >= _amount
            ...
            accounts[_to].balance = (_amount + (accounts[_to].balance)).toUINT112();
    ...
    function mint(address _owner, uint256 _amount, bool _isRaw, uint32 timestamp) onlyOwner{
        if (_isRaw) {
            accounts[_owner].rawTokens = (_amount + (accounts[_owner].rawTokens)).toUINT112();
            supplies.rawTokens = (_amount + (supplies.rawTokens)).toUINT128();
        } else {
            accounts[_owner].balance = (_amount + (accounts[_owner].balance)).toUINT112();
        }

        accounts[_owner].lastMintedTimestamp = timestamp;

        supplies.total = (_amount + (supplies.total)).toUINT128();
        Transfer(0, _owner, _amount);
    }
    
    // Offer bonus to raw tokens holder
    function offerBonus(uint256 _bonus) onlyOwner { 
        bonusOffered = (bonusOffered + _bonus);
        supplies.total = (_bonus + (supplies.total)).toUINT128();
...
contract VENSale is Owned{
    ...
    uint256 public constant totalSupply         = 10 ** 27; // 1 billion VEN, decimals set to 18

    // inlined totalSupply
    uint256 constant privateSupply              = 9 * (10 ** 25);  // 9% for private ICO
    uint256 constant commercialPlan             = 23 * (10 ** 25); // 23% for commercial plan
    uint256 constant reservedForTeam            = 5 * (10 ** 25);  // 5% for team
    uint256 constant reservedForOperations      = 22 * (10 ** 25); // 22 for operations

    // 59%
    uint256 public constant nonPublicSupply     = privateSupply + commercialPlan + reservedForTeam + reservedForOperations;
    // 41%
    uint256 public constant publicSupply = (10 ** 9) * (10 ** 18) - nonPublicSupply;
    ...
    uint256 constant venPerEthEarlyStage = 3500 + 525 /* venPerEth + venPerEth * 15 / 100 */;  // early stage has 15% reward
    ...
    function stage() constant returns (Stage) { 
        ...
        if (((uint256(soldOut.official)) + (soldOut.channels)) >= publicSupply) {
            // all sold out
            return Stage.Closed;
        }
        ...
        if (blockTime() < endTime) {
            // in sale            
            if (blockTime() < (startTime + earlyStageLasts)) {
                // early bird stage
                return Stage.Early;
            }
            // normal stage
    ...
    function buy() payable {
        ...
        if (msg.value > maxBuyEthAmount) {
            requested = (maxBuyEthAmount * rate);
        } else {
            requested = ((msg.value) * rate);
        }

        uint256 remained = (officialLimit - (soldOut.official));
        if (requested > remained) {
            //exceed remained
            requested = remained;
        }

        uint256 ethCost = (requested / rate);
        if (requested > 0) {
            ven.mint(msg.sender, requested, true, blockTime());
            // transfer ETH to vault
            ethVault.transfer(ethCost);

            soldOut.official = (requested + (soldOut.official)).toUINT120();
            onSold(msg.sender, requested, ethCost);        
        }

        uint256 toReturn = ((msg.value) - ethCost);
        if(toReturn > 0) {
    ...
    function offerToChannel(address _channelAccount, uint256 _venAmount) onlyOwner {
        ...
        soldOut.channels = (_venAmount + (soldOut.channels)).toUINT120();
    ...
    function initialize(
        ...
        address o = _ven.owner();
        require(o == address(this));
        ...
        ven.mint(
            venVault,
            (reservedForTeam + reservedForOperations),
    ...
    function finalize() onlyOwner {
        ...
        uint256 unsold = (((publicSupply - (soldOut.official))) - (soldOut.channels));
