...
contract CustodianUpgradeable is LockRequestable {
    ...
    mapping (uint256 => CustodianChangeRequest) public custodianChangeReqs;
    ...
    function requestCustodianChange(address _proposedCustodian) public returns (bytes32 lockId) {
        ...
        custodianChangeReqs[uint(lockId)] = CustodianChangeRequest({
    ...
    function confirmCustodianChange(bytes32 _lockId) public onlyCustodian {
        ...
        delete custodianChangeReqs[uint(_lockId)];
    ...
    function getCustodianChangeReq(bytes32 _lockId) private view returns (address _proposedNew) {
        CustodianChangeRequest storage changeRequest = custodianChangeReqs[uint(_lockId)];
...
contract ERC20ImplUpgradeable is CustodianUpgradeable  {
    ...
    mapping (uint => ImplChangeRequest) public implChangeReqs;
    ...
    function requestImplChange(address _proposedImpl) public returns (bytes32 lockId) {
        ...
        implChangeReqs[uint(lockId)] = ImplChangeRequest({
    ...
    function confirmImplChange(bytes32 _lockId) public onlyCustodian {
        ...
        delete implChangeReqs[uint(_lockId)];
    ...
    function getImplChangeReq(bytes32 _lockId) private view returns (ERC20Impl _proposedNew) {
        ImplChangeRequest storage changeRequest = implChangeReqs[uint(_lockId)];
...
contract ERC20Impl is CustodianUpgradeable {
    ...
    mapping (uint => PendingPrint) public pendingPrintMap;
    ...
    function requestPrint(address _receiver, uint256 _value) public returns (bytes32 lockId) {
        ...
        pendingPrintMap[uint(lockId)] = PendingPrint({
            receiver: _receiver,
            value: _value
        });
    ...
    function confirmPrint(bytes32 _lockId) public onlyCustodian {
        PendingPrint storage print = pendingPrintMap[uint(_lockId)];
        ...
        delete pendingPrintMap[uint(_lockId)];
    ...
    function burn(uint256 _value) public returns (bool success) {
        ...
        uint t = erc20Store.totalSupply();
        erc20Store.setTotalSupply(t - _value);
    ...
    function totalSupply() public view returns (uint256) {
        uint256 t = erc20Store.totalSupply();
        return t;
    }

    /// @notice  Core logic of the ERC20 `balanceOf` function.
    function balanceOf(address _owner) public view returns (uint256 balance) {
        uint256 b = erc20Store.balances(_owner);
        return b;
    }

    /// @notice  Core logic of the ERC20 `allowance` function.
    function allowance(address _owner, address _spender) public view returns (uint256 remaining) {
        uint256 a = erc20Store.allowed(_owner, _spender);
        return a;
    }
